    ...
    return (
        <div className={styles.container}>
            ...
            <div className={styles.navPlacesCountries}>
                {countries.map(country => <div onClick={() => handleCountryChange(country)} className={`${styles.navPlacesItem} ${currentState.currentCountry === country && styles.activeNavPlacesItem}`}>{country}</div>)}
            </div>
            <div className={styles.cardsContainer}>
                {
                    places.length === 0 ?
                        <h3>No {title} Yet</h3>
                        :
                        currentState.currentPlaces.map(place => <PlaceCard id={place.id} cityName={place.cityName} country={place.country} date={place.date} notes={place.notes} position={place.position} status={place.status} countryCode={place.countryCode} />)
                }
            </div>
        </div>
    )
