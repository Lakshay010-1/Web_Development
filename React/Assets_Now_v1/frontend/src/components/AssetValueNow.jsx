...
function assetHisResultReducer(state, action) {
    ...
    switch (action.type) {
        ...
        case "setData":
            ...
            newState.dataDiffPer = action.payload.data.dataDiffPer;
            break;
        ...
        async function fetchData() {
            try {
                ...
                if (!data.data?.apiLimit && assetSearchResultState.type != "Currency") {
                    ...
                    data.data.dataDiffPer = parseFloat(((data.data.toPrice.price - data.data.fromPrice.price) * 100 / data.data.fromPrice.price))
                }
    ...
    return (
        <AssetTab className="asset-value-now" title="Assets Value Now" openedTab={props.openedTab === props.id} handleClick={props.handleOpenedTab} id={props.id} >
            <div id="asset-value-now">
                ...
                {activeStep == 2 &&
                    <ValueNowStep3 user={props.user} setPortfolioList={props.setPortfolioList} selectedAssetHistoricalResult={assetHistResultState} selectedInvestedCurrencyOption={assetSearchResultState.currencyOption} selectedSearchAssetHisType={assetSearchResultState.type} selectedSearchAssetHisValue={assetSearchResultState.value} selectedSearchAssetHisSymbol={assetHistResultState.symbol} />
                }
