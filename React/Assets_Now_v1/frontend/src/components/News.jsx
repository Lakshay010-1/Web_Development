...
export default function News(props) {
    ...
    const totalPages = Math.ceil(props.news.data.length / newsItemPerPage);


    return <AssetTab title="Finance News" openedTab={props.openedTab === props.id} handleClick={props.handleOpenedTab} id={props.id}>
        <div id="finance-news" >
            {props.news.status == "loading"
                ?
                <Loader message="News" />
                :
                <>
                    <NewsPage newsList={props.news.data} newsItemPerPage={newsItemPerPage} currentNewsPage={currentNewsPage} />
