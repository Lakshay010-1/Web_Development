...
import LoginForm from './components/AuthForm.jsx'
...
const userInitialState = { user: {}, loading: true, isAuthenticated: false };
...
export default function App() {
    ...
    async function fetchNews() {
      ...
      if (!storedNews?.news || storedNews.news?.error) {
        try {
          ...
          const response = await fetch(`${APILINK}/api/fetch/news`);
          let data = await response.json();
  ...
  // Portfolio and Watchlist Items
  useEffect(() => {
    async function fetchItems(list) {
      const response = await fetch(`${APILINK}/api/list/${list}?user=${user.user.email}`);
      const data = await response.json();
      if (response.ok) {
        if (list === "p") {
          setPortfolioList(data.assets);
        } else if (list === "w") {
          setWatchlistList(data.assets);
        }
      }
    }
    if (user !== undefined) {
  ...
  useEffect(() => {
    async function checkAuth() {
      try {
        ...
        const response = await fetch(`${APILINK}/me`, {
          credentials: "include",
        });
        const data = await response.json();
      ...
      <div id='container'>
        {
          user.loading ?
            ...
            user.isAuthenticated ?
              <>
                <Header user={user} userDispatch={userDispatch} currenciesList={currenciesListState.data} setSearchAssetType={assetSearchResultDispatch} setSearchAssetValue={assetSearchResultDispatch} loadingCurrencyListStatus={currenciesListState.status} />
                <div id="content-container">
                  <TrendingItems trendingItems={trendingItemsState} />
                  <div id="content">
                    <SearchResult user={user} loadingSearchResult={assetSearchResultState.status} docTitle={tabState.title} setDocTitle={tabDispatch} searchAssetType={assetSearchResultState.type} setWatchlistList={setWatchlistList} data={assetSearchResultState.data} searchAssetValue={assetSearchResultState.value} usdToINR={usdToINR} openedTab={tabState.openedTabID} handleOpenedTab={tabDispatch} id={ids[0]} />
                    <AssetValueNow user={user} docTitle={tabState.title} setDocTitle={tabDispatch} currenciesList={currenciesListState.data} setPortfolioList={setPortfolioList} usdToINR={usdToINR} openedTab={tabState.openedTabID} handleOpenedTab={tabDispatch} id={ids[1]} />
                    <Portfolio portfolioList={portfolioList} openedTab={tabState.openedTabID} handleOpenedTab={tabDispatch} setPortfolioList={setPortfolioList} id={ids[2]} />
                    <Watchlist watchlistList={watchlistList} openedTab={tabState.openedTabID} handleOpenedTab={tabDispatch} setWatchlistList={setWatchlistList} id={ids[3]} />
