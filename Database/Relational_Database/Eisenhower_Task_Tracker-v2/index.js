...
const API_LINK = "http://localhost:8889";
...
app.get("/", async (req, res) => {
    try {
        ...
        let { data: tasks } = await axios.get(`${API_LINK}/tasks/space/${currentSpace}`);

        for (let i = 0; i < tasks.length; i++) {
            const date = new Date(tasks[i].duedate);
            const local = new Date(date.getTime() - date.getTimezoneOffset() * 60000);
            tasks[i].duedate = local.toISOString().slice(0, 16);
        }
...
app.get("/toggleLayout", async (req, res) => {
    try {
        ...
        res.json({ "message": "Error toggling current layout" });
    }
});
...
app.get("/toggleTheme", async (req, res) => {
    try {
        ...
        res.json({ "message": "Error toggling current theme" });
    }
});
...
app.get("/tasks/patch", async (req, res) => {
    ...
    try {
        ...
        const local = new Date(date.getTime() - date.getTimezoneOffset() * 60000);
        task.duedate = local.toISOString().slice(0, 16);
